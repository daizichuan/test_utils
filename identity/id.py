...
# 导入常量并重命名
from identity.constant import Constant
...
class IdNumber(str):
    ...
    def get_areaName_from_areaId(self, area_id):
        """根据区域编号取出区域名称"""
        return Constant.AREA_INFO[area_id]
    ...
    def verify_id(cls, id_number):
        ...
        if re.match(Constant.ID_NUMBER_18_REGEX, id_number):
            ...
            return 'False' if bool(str(check_digit) == id_number[-1].upper()) == False else cls(
                id_number).get_areaName_from_areaId(int(id_number[0:6]))
        else:
            return 'False' if bool(re.match(Constant.ID_NUMBER_15_REGEX, id_number)) == False else cls(
                id_number).get_areaName_from_areaId(int(id_number[0:6]))
